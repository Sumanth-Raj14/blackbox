...
async def test_routing_api_list(client, auth_headers):
    resp = await client.get("/api/v1/manufacturing/", headers=auth_headers)
    assert resp.status_code in (200, 401, 403)


@pytest.mark.asyncio
async def test_routing_api_create(client, auth_headers):
    resp = await client.post("/api/v1/manufacturing/", headers=auth_headers, json={"name": "test"})
...
async def test_routing_api_without_auth(client):
    resp = await client.get("/api/v1/manufacturing/")
