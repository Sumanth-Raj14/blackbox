...
6. **Migrate** â€” container entrypoint (or orchestrator) runs `alembic upgrade head` with a DB-wait/retry (31 revisions), with `SKIP_CREATE_ALL=true` to bypass the deprecated `create_all` (`main.py:81-90`).
...
**Re-run safety:** `.env` preserved (never rotated â€” rotating `SECRET_KEY` would invalidate JWTs/encrypted data), compose converges declaratively, `alembic upgrade head` no-ops, seed/admin skip when data/user already exist (`seed_db.py:220-304`, `create_admin.py:59-61`). Partial-failure resume must be safe.
...
| First-run bootstrap orchestrator (idempotent) | Single cross-platform script (Python 3, already required): ensure `.env` â†’ `docker compose up -d --build` â†’ poll `/api/v1/health` â†’ `alembic upgrade head` â†’ tenant+admin bootstrap â†’ print URL + open browser. Guard every step for no-op re-runs. | NEW `backend/scripts/bootstrap.py`; consumed by all three launchers | **L** | `.env` generator, entrypoint/migrate, admin bootstrap |
...
| Container entrypoint that migrates before serving | Entrypoint runs `alembic upgrade head` (with DB-wait/retry) then execs uvicorn; set `SKIP_CREATE_ALL=true` in compose to disable deprecated `create_all` (`main.py:81-90`). Self-migrating image; removes the manual `docker compose exec` step. | `backend/Dockerfile` & `Dockerfile.prod` (ENTRYPOINT), NEW `backend/scripts/docker-entrypoint.sh`, `docker-compose.yml` | **M** | None |
